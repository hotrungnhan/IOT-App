...
namespace MQTTnet.AspNetCore.Client.Routing.Lib
{
    ...
    {
        private static readonly ConcurrentDictionary<Key, MqttRouteTable> Cache =
            new ConcurrentDictionary<Key, MqttRouteTable>();
        ...
        internal static MqttRouteTable Create(IEnumerable<MethodInfo> actions)
        {
            ...
            {
                var routeAttributes = action.GetCustomAttributes<MqttEventSubscribeAttribute>(inherit: false)
                    .SelectMany(s => s.TopicFilters)
                    .Select(a => ReplaceTokens(a.Topic, action.DeclaringType.Name, action.Name))
                    .ToArray();
        ...
        internal static MqttRouteTable Create(Dictionary<MethodInfo, string[]> templatesByHandler)
        {
            ...
            foreach (var keyValuePair in templatesByHandler)
            {
                ...
                foreach (var parsedTemplate in parsedTemplates)
                {
                    ...
                    // var mrat = methodInfo.DeclaringType.GetCustomAttribute<MqttEventSubscribeAttribute>();
                    // if (mrat != null)
                    // {
                    //     entry.ControllerTemplate = TemplateParser.ParseTemplate(mrat);
        ...
        internal static int RouteComparison(MqttRoute x, MqttRoute y)
        {
            ...
            if (xTemplate.Segments.Count != y.Template.Segments.Count)
            {
                ...
                if (!xTemplate.Segments[xTemplate.Segments.Count - 1].IsCatchAll &&
                    yTemplate.Segments[yTemplate.Segments.Count - 1].IsCatchAll)
                {
                    return -1;
                }
